...
class OpenYuanRongSandboxBackendTest(unittest.TestCase):
    ...
    def test_snapshot_mode_native_error_does_not_invalidate_generation(self):
        native = MagicMock()
        native.id = "default-authoritative-snapshot-native-error"
        ...
        native.reload.side_effect = reload_native
        handle = MagicMock(pid=321)
        handle.wait.side_effect = [
            RuntimeError("transient native failure"),
            SimpleNamespace(stdout="restored\n", stderr="", exit_code=0),
        ]
        native.commands.run.return_value = handle
        session = self._create_session(native)
        pid = self._start(session)
        ...
        with self.assertRaisesRegex(
            BackendOperationError,
            "wait for process 321 failed: transient native failure",
        ) as raised:
            session.commands.wait(pid, 30)
        self.assertNotIn(_COLD_START_HANDLE_ERROR, str(raised.exception))
        self.assertEqual(
            session.commands.wait(pid, 30),
            CommandResult("restored\n", "", 0),
        )
