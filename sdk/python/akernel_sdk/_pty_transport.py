...
def _build_pty_uri(
    ...
    *,
    ...
    return (
        f"{endpoint.websocket_scheme}://"
        f"{endpoint.authority(omit_default_port=True)}/terminal/ws?"
...
class _PtyConnection:
    ...
    def wait(self, timeout: float | None) -> int:
        if not self._done.wait(timeout):
            ...
            raise TimeoutError(f"PTY did not exit within {rendered}")
        self._raise_error()
    ...
    def _send(self, data: bytes | str) -> None:
        ...
        try:
            ...
            raise _PtyTransportError(f"failed to write to PTY: {error}") from error
    ...
    async def _run(self) -> None:
        ...
        try:
                    ...
                    if isinstance(message, bytes):
                        if self._on_data is not None:
                            try:
                                ...
                                self._set_error(
                                    _PtyTransportError(
                                        f"PTY on_data callback failed: {error}"
                                    )
                                )
            ...
            if not self._closed_by_client and not self._done.is_set():
                self._set_error(
                    _PtyTransportError(f"PTY WebSocket connection failed: {error}")
                )
        finally:
            with self._state_lock:
    ...
    def _handle_control(self, message: str) -> None:
        try:
            ...
            self._set_error(_PtyTransportError(f"invalid PTY control frame: {error}"))
            return
