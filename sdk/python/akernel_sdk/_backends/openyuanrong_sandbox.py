...
class _CommandsDriver:
    def __init__(self, commands: Any) -> None:
        ...
        self._uncertain_generations: set[int] = set()
            ...
            if reloaded:
                ...
                with self._generation_lock:
                    ...
                    if mode == "cold-start":
                        ...
                        self._uncertain_generations.update(range(self._generation))
            return reloaded
        finally:
            with self._operation_state_lock:
                self._reload_in_progress = False
    ...
    def _invalidate_pre_reload_generation(
        ...
        with self._generation_lock:
            if (
                pid.generation >= self._generation
                or pid.generation not in self._uncertain_generations
            ):
                return False
